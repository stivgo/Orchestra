<template>
  <div>
    <b-modal :id="'edit-text'+(element.indexS?element.indexS:element._id)" hide-footer centered title="Editar Texto">
        <form class="row g-3" @submit="updateText" @submit.prevent>
            ...
            <div class="modal-footer">
                <button @click="$bvModal.hide('edit-text'+(element.indexS?element.indexS:element._id))"
...
export default {
  ...
  methods: {
    ...
    async updateText() {
      ...
      this.$bvModal.hide('edit-text'+(this.element.indexS?this.element.indexS:this.element._id))
    },
    addStyle(){
