<template>
  <div>
    <b-modal :id="'delete-video'+(element.indexS?element.indexS:element._id)" hide-footer centered title="Eliminar Video">
      <form @submit="deleteElement" @submit.prevent>
        <div class="modal-body">
          <p>
            ¿Esta seguro en eliminar el video?
            <br/>
            {{ element.title }} 
          </p>
        </div>
        <div class="modal-footer">
          <button type="button" @click="$bvModal.hide('delete-video'+(element.indexS?element.indexS:element._id))" class="btn btn-secundary">
            Cerrar
          </button>
...
export default Vue.extend({
  ...
  methods: {
    ...
    async deleteElement() {
      ...
      this.$bvModal.hide('delete-video'+(this.element.indexS?this.element.indexS:this.element._id));
    },
  }
});
</script>
