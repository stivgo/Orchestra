<template>
  <div>
    <b-modal :id="'edit-video'+(element.indexS?element.indexS:element._id)" hide-footer centered title="Editar Video">
        <form class="row g-3" @submit="updateText" @submit.prevent>
            ...
            <div class="modal-footer">
                <button @click="$bvModal.hide('edit-video'+(element.indexS?element.indexS:element._id))"
...
export default {
  ...
  methods: {
    ...
    async updateText() {
      ...
      this.$bvModal.hide('edit-video'+(this.element.indexS?this.element.indexS:this.element._id))
    },
    addStyle(){
