<template>
  <div>
    <b-modal :id="'edit-link'+(element.indexS?element.indexS:element._id)" hide-footer centered title="Editar Enlace">
        <form class="row g-3" @submit="updateText" @submit.prevent>
            <div class="modal-body">
                ...
                <div class="input-group mb-2">
                    <label class="input-group-text w-25">Url</label>
                    <input class="form-control" type="text" required v-model="element.urlSource" />
                </div>
                <div class="input-group  mb-2">
                    <label class="input-group-text w-25" for="">Contenido</label>
                    <textarea class="form-control" name="textarea" rows="5" cols="35" v-model="element.content" required> </textarea>
                </div>
                <div class="input-group  mb-2">
            ...
            <div class="modal-footer">
                <button @click="$bvModal.hide('edit-link'+(element.indexS?element.indexS:element._id))"
...
export default {
  ...
  methods: {
    ...
    async updateText() {
      ...
      this.$bvModal.hide('edit-link'+(this.element.indexS?this.element.indexS:this.element._id))
    },
    addStyle(){
