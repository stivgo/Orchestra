<template>
  <div>
    <b-modal :id="'insert-audio'" hide-footer centered title="Nuevo audio">
        <form class="row g-3" @submit="createText" @submit.prevent>
            <div class="modal-body">
                ...
                <div class="input-group mb-2">
                    <label class="input-group-text w-25" for="">Url</label><input
                        class="form-control" type="text" required v-model="form.urlSource">
                </div>
                <div class="input-group mb-2">
                    <label class="input-group-text w-25" for="">Tipo de audio</label>
                    <select class="form-select" v-model="form.audioType" required>
                        <option value="">Seleccione el tipo de audio</option>
                        <option v-for="type of audioTypes" :key="type.value" :value="type.value">
                            {{type.name}}
                        </option>
                    </select>
                </div>
            ...
            <div class="modal-footer">
                <button @click="$bvModal.hide('insert-audio')" type="button" class="btn btn-secundary">
                    Cerrar
                </button>
                <button type="submit" class="btn btn-primary">Crear audio</button>
            </div>
        </form>
    </b-modal>
  </div>
...
export default {
  ...
  methods: {
    ...
    async createText() {
      ...
      this.resetForm()
      this.$bvModal.hide('insert-audio')
    },
    resetForm(){
        this.form.pageId= null,
        this.form.type= "Paragraph",
        this.form.title= "",
