<template>
  <div>
    <b-modal :id="'delete-audio'+(element.indexS?element.indexS:element._id)" hide-footer centered title="Eliminar Audio">
      <form @submit="deleteElement" @submit.prevent>
        <div class="modal-body">
          <p>
            ¿Esta seguro en eliminar el audio?
            <br/>
            {{ element.title }} 
          </p>
        </div>
...
export default Vue.extend({
  ...
  methods: {
    ...
    async deleteElement() {
      ...
      this.$bvModal.hide('delete-audio'+(this.element.indexS?this.element.indexS:this.element._id));
    },
  }
});
</script>
